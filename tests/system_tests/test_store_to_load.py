...
async def test_store_to_load_hazard(dut):
    ...
    print(f"Loading {len(instr_mem)} instructions into memory...")
    for i, instr in enumerate(instr_mem):
        dut.instr_mem_inst.instr_ram[i].value = instr

    await RisingEdge(dut.clk)  # Let memory writes settle

    cycle = 0
    max_cycles = 500  # Increased to account for cache miss delays

    print(f"\nRunning program for {max_cycles} cycles...")

    for cycle in range(max_cycles):
        # Data memory is handled by data_mem module in top
        # No need to manually feed instructions - cache does this

        await RisingEdge(dut.clk)

        # Detailed debug logging for key cycles
        if (10 <= cycle <= 50) or (cycle % 20 == 0):  # Early cycles + every 20th cycle
            try:
                pc_val = int(dut.cpu_inst.module_pc_out.value)
                cache_stall = int(dut.cache_stall.value)

                # Cache signals - can now access through top module
                try:
                    cache_pc_reg = -99  # Internal to cache, would need special access
                    cache_output = int(dut.instr_to_cpu.value)
                    cache_valid = 1 if cache_stall == 0 else 0
                except:
                    cache_pc_reg = -99
                    cache_output = -99
                    cache_valid = -99

                # IF_ID signals (DUT is now top, so access through cpu_inst)
                try:
                    if_id_instr_in = int(dut.cpu_inst.if_id_inst0.instruction_in.value)
                    if_id_instr_out = int(dut.cpu_inst.if_id_inst0.instruction_out.value)
                    if_id_enable = int(dut.cpu_inst.if_id_inst0.enable.value)
                except:
                    if_id_instr_in = -1
                    if_id_instr_out = -1
                    if_id_enable = -1
                ...
                # Stall signals
                try:
                    load_use_stall = int(dut.cpu_inst.load_use_stall.value)
                except:
                    load_use_stall = -1

                print(f"C{cycle:02d}: PC=0x{pc_val:02x} cache_pc_reg=0x{cache_pc_reg:02x} " +
                      f"cache_out=0x{cache_output:08x} cache_valid={cache_valid} " +
                      f"if_id_in=0x{if_id_instr_in:08x} if_id_out=0x{if_id_instr_out:08x} " +
                      f"if_id_en={if_id_enable} lu_stall={load_use_stall} c_stall={cache_stall}")
            except Exception as e:
                print(f"C{cycle:02d}: Debug error: {e}")
    ...
    for i in range(10, 17):
        try:
            val = int(dut.cpu_inst.rf_inst0.register_file[i].value)
            results[i] = val
            print(f"x{i} = {val}")
        except:
